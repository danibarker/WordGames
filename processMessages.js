...
const processChat = (chat, chatlog) => {
	...
	newChatLog = `[${new Date().toLocaleString()}]\n${chat.username}: ${chat.message}\n`+newChatLog
	return newChatLog
}
const processReset = () => {
	let gameState = {
    ...
    console.log("gameState process reset",gameState)
...
const processLogout = (gameState,data) => {
    let newGameState = { ...gameState }
	newGameState.players = newGameState.players.filter((user) => user.name != data);
	newGameState.numPlayers = newGameState.players.length;
...
const processGuess = (gameState,data) => {
	...
	if (data.username === newGameState.players[newGameState.playerOnTurn].name) {
...
const processStart = (gameState) => {
	...
	if (!newGameState.currentWord) {
        ...
        console.log('processStart',newGameState)
    }
    saveGameState(newGameState)
	return JSON.stringify({ type: "START", data: newGameState });
};
module.exports = {processChat,processGuess,processLogin,processLogout,processReset,processReset,processStart}
